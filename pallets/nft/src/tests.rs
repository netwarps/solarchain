...
fn mint() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            SUT::token_by_id::<H256>(Vec::<u8>::default().blake2_256().into()).owner,
            0
        );
...
fn mint_err_max() {
    new_test_ext().execute_with(|| {
        ...
        assert_err!(
            SUT::mint(Origin::root(), 6, vec![4], None),
...
fn transfer() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            SUT::token_by_id::<H256>(tokens_for_account[0]).owner,
            2
        );
    });
}
