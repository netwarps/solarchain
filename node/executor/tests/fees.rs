...
fn fee_multiplier_increases_and_decreases_on_big_weight() {
	...
	let block1 = construct_block(
		...
		vec![
			...
			CheckedExtrinsic {
				signed: Some((charlie(), signed_extra(0))),
	...
	let block2 = construct_block(
		...
		vec![
			...
			CheckedExtrinsic {
				signed: Some((charlie(), signed_extra(1))),
	...
	loop {
		...
		let mut xts = (0..num_transfers)
			.map(|i| CheckedExtrinsic {
				signed: Some((charlie(), signed_extra(nonce + i as Index))),
	...
	loop {
		...
		let block = construct_block(
			...
			vec![
				...
				CheckedExtrinsic {
					signed: Some((charlie(), signed_extra(nonce))),
