...
use solar_node_runtime::{self, opaque::Block, RuntimeApi};
...
impl sc_executor::NativeExecutionDispatch for ExecutorDispatch {
	...
	fn dispatch(method: &str, data: &[u8]) -> Option<Vec<u8>> {
		solar_node_runtime::api::dispatch(method, data)
	}

	fn native_version() -> sc_executor::NativeVersion {
		solar_node_runtime::native_version()
	}
}
...
> {
	...
	Ok(sc_service::PartialComponents {
		...
		other: (
			telemetry,
			(frontier_block_import, block_import, grandpa_link, babe_link),
			frontier_backend,
			filter_pool,
...
pub fn new_full(mut config: Configuration, cli: &Cli) -> Result<TaskManager, ServiceError> {
	...
	let warp_sync = Arc::new(sc_finality_grandpa::warp_proof::NetworkProvider::new(
		backend.clone(),
		consensus_result.2.shared_authority_set().clone(),
		Vec::default(),
	));
	...
	let (_frontier_block_import, block_import, grandpa_link, babe_link) = consensus_result;
